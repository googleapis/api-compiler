...
public class diff_match_patch {
  ...
  /**
   * Ensure UTF-16 surrogate pairs do not get split across boundaries.
   * diff_match_patch works with Java chars for performance reasons, but can therefore split text
   * in the middle of a UTF-16 surrogate pair. This method cleans up such split pairs.
   * You want to invoke this if your text could contain code points above U+FFFF.
   * Invoke it after everything else; other methods could reintroduce split pairs.
   *
   * @param diffs LinkedList of Diff objects.
   */
  public void diff_cleanupSurrogates(LinkedList<Diff> diffs) {
    ListIterator<Diff> pointer = diffs.listIterator();
    while (pointer.hasNext()) {
      Diff thisDiff = pointer.next();
      // For each EQUAL diff we look for misplaced surrogates at the beginning and end, and move
      // them to neighbouring INSERT and DELETE diffs.
      if (thisDiff.operation == Operation.EQUAL) {
        char start = diff_firstChar(thisDiff);
        char end = diff_lastChar(thisDiff);
        // Low surrogate at the start, move to the end of previous INSERT / DELETEs.
        if (Character.isLowSurrogate(start)) {
          thisDiff.text = thisDiff.text.substring(1);
          int savedPlace = pointer.nextIndex();
          // Spin back and add to every diff ending with a high surrogate we find.
          pointer.previous();
          while (pointer.hasPrevious()) {
            Diff prevDiff = pointer.previous();
            if (Character.isHighSurrogate(diff_lastChar(prevDiff))) {
              assert prevDiff.operation != Operation.EQUAL;
              prevDiff.text = prevDiff.text + start;
            } else {
              break;
            }
          }
          // Get back where we were.
          while (pointer.nextIndex() < savedPlace) {
            pointer.next();
          }
        }
        // High surrogate at the end, move to the start of following INSERT / DELETEs.
        if (Character.isHighSurrogate(end)) {
          thisDiff.text = thisDiff.text.substring(0, thisDiff.text.length() - 1);
          int savedPlace = pointer.nextIndex();
          // Spin back and add to every diff starting with a low surrogate we find.
          while (pointer.hasNext()) {
            Diff nextDiff = pointer.next();
            if (Character.isLowSurrogate(diff_firstChar(nextDiff))) {
              assert nextDiff.operation != Operation.EQUAL;
              nextDiff.text = end + nextDiff.text;
            } else {
              break;
            }
          }
        ...
        // If we reduced our EQUAL down to "", remove it.
        if (thisDiff.text.isEmpty()) {
          pointer.remove();
        }
      }
    }
    // If we removed any EQUAL diffs above we might have sequences like INSERT DELETE DELETE.
    // Merge anything that needs merging.
    pointer = diffs.listIterator();
    Diff lastInsert = null;
    Diff lastDelete = null;
    while (pointer.hasNext()) {
      Diff thisDiff = pointer.next();
      switch (thisDiff.operation) {
        case EQUAL:
        ...
        case DELETE:
          if (lastDelete == null) {
            ...
            lastDelete.text = lastDelete.text + thisDiff.text;
            pointer.remove();
          }
          break;
        case INSERT:
          if (lastInsert == null) {
            ...
            lastInsert.text = lastInsert.text + thisDiff.text;
            pointer.remove();
          }
          break;
      }
  ...
  private char diff_firstChar(Diff diff) {
    return diff.text.charAt(0);
  }

  private char diff_lastChar(Diff diff) {
    return diff.text.charAt(diff.text.length() - 1);
  }
